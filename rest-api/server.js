...
server.route({
  ...
  handler: async (request, h) => {
    ...
    const response = await Vehicles
      .raw(`drop table if exists temp1;
create temp table if not exists temp1(application_id uuid,
                       vehicle_id uuid,
                       event_name varchar(45),
                       timestamp timestamp,
...
const insert = await Vehicles
  .raw(`
    select null as vehicle_id, a.name as application_name, sum(total_time) as total_time, sum(uses) as uses, max(max_time) as max_time,min(min_time) as min_time, sqrt(sum(variance)) as std_dev from usage_over_time
    join applications a on a.id = application_id
    group by a.name`);
...
const insert = await Vehicles
  .raw(`
    select distinct v.name as vehicle_name, a.name as application_name, total_time, uses, max_time, min_time, avg_time, std_dev, variance, day, week, month, year from usage_over_time
